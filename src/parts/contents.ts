...
export class Contents extends MyDisplay {
  ...
  constructor(opt:any) {
    ...
    img.onload = () => {
      ...
      const text = Array.from('BEER');
      let innerTxt = '';
      this._posTable.forEach((val,i) => {
        const t = text[i % text.length];
        innerTxt += '<span data-color="' + val.color + '" data-order="' + val.d + '">' + t + '</span>';
        if((i + 1) % this._col == 0) {
          innerTxt += '<br>';
        }
      });
  ...
  protected _update(): void {
    ...
    this._item.forEach((val,i) => {
      ...
      if(isFix) {
        // val.el.classList.add('-fix')
        Tween.instance.set(val.el, {
          backgroundColor:val.color,
          color:val.color
        })
      } else {
        // val.el.classList.remove('-fix')
        Tween.instance.set(val.el, {
          backgroundColor:'',
          color:'#062418'
        })
